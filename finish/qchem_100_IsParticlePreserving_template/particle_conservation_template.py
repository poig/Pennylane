...
def binary_list(m, n):
    ...
    arr = []
    # QHACK #
    arr = np.array([int(a) for a in str("{0:0{1}b}".format(m,n))])
    # QHACK #
    return arr
...
def is_particle_preserving(circuit, n):
    ...
    circuit_list = []
    for i in range(2**n):
        circuit_list.append(circuit(binary_list(i,n)))
        if not(circuit_list[i][i] == circuit_list[0][0]):
            return True
    ...
    '''store = []
    for i in range(2**n):
        store.append(circuit_list[i] == binary_list(i,n))'''
    
    '''Hamming_weight= []
    for i in range(len(basis_state)):
        Hamming_weight.append(circuit(basis_state[i]))'''
