...
def superdense_coding(bits, alpha):
    ...
    #qml.PauliRot(alpha, 'X',  wires=0)
    #qml.MottonenStatePreparation(state_vector=np.array([np.cos(alpha),np.sin(alpha)]),wires=range(1))
    #qml.QubitUnitary([np.cos(alpha),np.sin(alpha)] ,wires=0)
